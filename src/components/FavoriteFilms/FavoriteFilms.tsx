...
export default function FavoriteFilms() {
  ...
  const [selectedCustomers, setSelectedCustomers] = useState<Films | null>(
    null
  );
  ...
  const imageBodyTemplate = (item: Films) => {
    return (
      <Link to={`/ra-redux-toolkit-ts/film-card/${item.imdbID}`}>
        <img
          src={item.Poster}
          alt={item.Title}
          width="64px"
    ...
    <div className="card">
      ...
      >
        <div className="confirmation-content">
          ...
          {selectedCustomers && (
            <span>
              Вы уверены, что хотите удалить из избранного{" "}
              <b>{selectedCustomers.Title}</b>?
            </span>
          )}
        </div>
      </Dialog>
